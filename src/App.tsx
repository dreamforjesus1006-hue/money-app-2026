            ...
            <table className="w-full text-sm text-left">
                      ...
                      <tr className="bg-slate-900/80 animate-in fade-in">
                        <td colSpan={11} className="p-4 border-b border-emerald-900/30">
                          ...
                          <div className="mb-6 pt-2 border-b border-slate-700 pb-6 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
                            <div>
                                <div className="text-xs text-slate-400 mb-2 font-bold flex items-center gap-2">🏠 本月生活費 (留白則套用預設)</div>
                                <div className="flex items-center gap-2 bg-slate-900 p-2 rounded-lg border border-slate-700">
                                    <span className="text-slate-500 text-xs w-16 text-center">實際花費</span>
                                    <input 
                                        type="number" 
                                        placeholder={`預設 ${formatMoney(taxStatus.livingExpenses)}`}
                                        value={monthlyRecords[`${selectedYear}_${r.month}`]?.livingExpense ?? ''}
                                        onChange={e => updateMonthlyRecord(selectedYear, r.month, 'livingExpense', e.target.value === '' ? undefined : safeNum(e.target.value))}
                                        className="w-full bg-slate-800 border border-slate-600 rounded px-3 py-1.5 text-white outline-none focus:border-emerald-500 transition-colors"
                                        onClick={e => e.stopPropagation()}
                                    />
                                </div>
                            </div>
                            <div>
                                <div className="text-xs text-slate-400 mb-2 font-bold flex items-center gap-2">💰 本月其他收入 (如: 兼職、租金)</div>
                                <div className="flex flex-col gap-2 bg-slate-900 p-2 rounded-lg border border-slate-700">
                                    <div className="flex items-center gap-2">
                                        <span className="text-slate-500 text-xs w-16 text-center">收入金額</span>
                                        <input 
                                            type="number" 
                                            placeholder="0"
                                            value={monthlyRecords[`${selectedYear}_${r.month}`]?.otherIncome ?? ''}
                                            onChange={e => updateMonthlyRecord(selectedYear, r.month, 'otherIncome', e.target.value === '' ? undefined : safeNum(e.target.value))}
                                            className="w-full bg-slate-800 border border-slate-600 rounded px-3 py-1.5 text-blue-300 font-bold outline-none focus:border-blue-500 transition-colors"
                                            onClick={e => e.stopPropagation()}
                                        />
                                    </div>
                                    <label className="flex items-center gap-2 text-xs text-slate-400 cursor-pointer pl-[72px]" onClick={e => e.stopPropagation()}>
                                        <input 
                                            type="checkbox"
                                            checked={monthlyRecords[`${selectedYear}_${r.month}`]?.isTaxable ?? false}
                                            onChange={e => updateMonthlyRecord(selectedYear, r.month, 'isTaxable', e.target.checked)}
              ...
              <tfoot>
                <tr className="bg-slate-900 font-black text-white">
                  ...
                  <td className="p-3 text-blue-300">{formatMoney(totalOtherIncome)}</td>
              ...
              <div className="pt-4 border-t border-slate-700 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2 p-3 bg-yellow-900/20 rounded-xl border border-yellow-700/50">
                    <div>
                      <label className="text-slate-400 flex items-center gap-1">年薪資所得 <span className="text-[10px] text-slate-500">(僅計稅，不列入現金流)</span></label>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      <label className="text-slate-400">預設每月生活費</label>
              ...
              <div className="pt-4 border-t border-slate-700">
                ...
                <button
                  onClick={() =>
                    setLoans((prev) => [
                      ...prev,
                      { id: Date.now().toString(), name: '新房貸', principal: 0, rate1: 2.1, rate1Months: 36, rate2: 2.3, totalMonths: 360, paidMonths: 0, gracePeriod: 0, type: 'PrincipalAndInterest' },
                    ])
                  }
