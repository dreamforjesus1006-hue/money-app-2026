...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      localStorage.setItem('baozutang_v53', JSON.stringify(state));
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-slate-900 text-white font-sans">
      <header className="mb-8 border-b border-slate-700 pb-4 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-emerald-400 flex items-center gap-2"><Calculator/> 包租唐戰情室 V53</h1>
              ...
              {etfs.map((e, idx) => (
                <div key={e.id} className="p-4 bg-slate-900 rounded-xl border border-slate-700 relative group">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                    ...
                    <div><label className="text-slate-500">現價</label><input type="number" value={e.currentPrice} onChange={v => { const n=[...etfs]; n[idx].currentPrice=Number(v.target.value); setEtfs(n); }} className="w-full bg-slate-800 rounded p-1 mt-1 border border-slate-700"/></div>
          ...
          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 w-full max-w-2xl shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-6 text-sm">
                  ...
                  {loans.map((l, i) => (
                    <div key={l.id} className="mb-4 p-3 bg-slate-900 rounded-xl border border-slate-700">
                        <div className="grid grid-cols-2 gap-3 mb-2">
                            <div><label className="text-slate-500 text-[10px]">貸款名稱</label><input type="text" value={l.name} onChange={e => updateLoan(i, 'name', e.target.value)} className="w-full bg-slate-800 p-1 rounded border border-slate-700"/></div>
                            <div><label className="text-slate-500 text-[10px]">本金金額</label><input type="number" value={l.principal} onChange={e => updateLoan(i, 'principal', Number(e.target.value))} className="w-full bg-slate-800 p-1 rounded border border-slate-700"/></div>
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                            <div><label className="text-slate-500 text-[10px]">寬限期(月)</label><input type="number" value={l.gracePeriod} onChange={e => updateLoan(i, 'gracePeriod', Number(e.target.value))} className="w-full bg-slate-800 p-1 rounded border border-slate-700"/></div>
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-2">
                            ...
                            <div><label className="text-slate-500 text-[10px]">一段期間(月)</label><input type="number" value={l.rate1Months} onChange={e => updateLoan(i, 'rate1Months', Number(e.target.value))} className="w-full bg-slate-800 p-1 rounded border border-slate-700"/></div>
                ...
                <div className="pt-4 border-t border-slate-700 grid grid-cols-2 gap-4">
                    <div><h4 className="text-blue-400 font-bold mb-2">信貸設定</h4>
