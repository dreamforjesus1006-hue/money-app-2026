...
// --- Defaults ---
const APP_SCHEMA_VERSION = 99; const LOCAL_KEY = 'baozutang_local';
...
export default function App() {
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    try { return sessionStorage.getItem('baozutang_auth') === 'true'; } catch(e) { return false; }
  });
  ...
  const handleLogin = () => {
    if (pwdInput === '085012') {
      try { sessionStorage.setItem('baozutang_auth', 'true'); } catch(e) {}
  ...
  const handleLogout = () => {
    try { sessionStorage.removeItem('baozutang_auth'); } catch(e) {}
  ...
  const tOut = monthlyFlows.reduce((a, b) => a + b.loanOut + b.creditOut + b.stockInt + b.fixedOut + b.life + b.healthTax + b.incomeTax, 0);
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 selection:bg-emerald-500/30">
        <div className="bg-slate-900/80 p-10 rounded-3xl shadow-2xl w-full max-w-sm text-center border border-slate-800">
          ...
          <input
            ...
            placeholder="請輸入通行密碼"
            className="w-full bg-slate-950 border border-slate-700 focus:border-emerald-500 rounded-xl px-4 py-3 text-white outline-none mb-6 text-center text-lg tracking-widest transition-colors"
          />
          <button
            onClick={handleLogin}
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-[0_0_15px_rgba(5,150,105,0.4)] flex justify-center items-center gap-2"
          >
            <Lock size={18}/> 解鎖進入
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-slate-950 text-white font-sans selection:bg-emerald-500/30">
      ...
      <header className="mb-6 border-b border-slate-800 pb-4 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-emerald-400 flex items-center gap-2 drop-shadow-md"><Calculator/> 包租唐戰情室 V99</h1>
