...
// ★★★★★ 請注意：這裡就是「鑰匙孔」 ★★★★★
// 請去您的 Firebase 後台複製設定，然後填入下面這幾個引號 "" 裡面
...
// --- Firebase Service (Init moved inside try-catch to allow no-config run) ---
let db: any = null;
const isFirebaseConfigured = YOUR_FIREBASE_CONFIG.apiKey && YOUR_FIREBASE_CONFIG.apiKey.length > 5 && !YOUR_FIREBASE_CONFIG.apiKey.includes("請貼上");
...
const STORAGE_KEY = 'baozutang_data_v45_online'; 
const CONFIG_KEY = 'baozutang_config';
const DOC_ID = 'baozutang_user_data'; // 單一使用者文件ID

const StorageService = {
    // 儲存：同時寫入 LocalStorage 和 Firestore
    ...
    // 載入：優先讀取 Cloud，失敗則讀 Local
    loadData: async () => { 
        try { 
            // 1. 嘗試讀取雲端
            if (isFirebaseConfigured && db) {
                const docSnap = await getDoc(doc(db, "users", DOC_ID));
                if (docSnap.exists()) {
                    return { data: docSnap.data(), source: 'cloud' };
                }
            }
            ...
            // 發生錯誤時退回備份
            return { data: USER_BACKUP_DATA, source: 'backup_error' }; 
        } 
    },
...
const App: React.FC = () => {
  ...
  const [dataSrc, setDataSrc] = useState<string>(''); // 顯示資料來源
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        setDataSrc(result.source); // 記錄資料來源
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 font-sans bg-slate-900 text-white selection:bg-emerald-500/30`}>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="bg-slate-800 rounded-2xl border border-slate-700 shadow-2xl w-full max-w-lg p-6">
                ...
                <div className="mb-4">
                    <label className="text-xs text-slate-400 block mb-1">Google Sheet CSV 連結</label>
                    <input type="text" value={cloudConfig.priceSourceUrl || ''} onChange={(e) => setCloudConfig({...cloudConfig, priceSourceUrl: e.target.value})} className="w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500 outline-none" />
                </div>
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-lg p-3 text-xs mb-4">
                    {isFirebaseConfigured ? <p className="text-emerald-300 font-bold flex items-center gap-2"><Wifi className="w-4 h-4"/> 雲端同步已連線</p> : <p className="text-slate-400 flex items-center gap-2"><WifiOff className="w-4 h-4"/> 離線模式 (請在代碼填入 Firebase Config)</p>}
                </div>
                <button onClick={() => setShowSettings(false)} className="w-full py-2 bg-slate-700 rounded hover:bg-slate-600 text-white">關閉</button>
            </div>
        </div>
      )}
      {showHelp && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"><div className="bg-slate-800 rounded-2xl border border-slate-700 shadow-2xl w-full max-w-xl p-6"><h3 className="text-xl font-bold mb-4">功能說明</h3><ul className="list-disc pl-5 space-y-2 text-slate-300 text-sm"><li><strong>雲端同步：</strong> 填入 Firebase Config 後，手機電腦可自動同步。</li></ul><button onClick={() => setShowHelp(false)} className="mt-4 w-full py-2 bg-slate-700 rounded hover:bg-slate-600">關閉</button></div></div>)}
      ...
      <header className="mb-8 border-b border-slate-700 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h1 className="text-3xl font-bold text-emerald-400 flex items-center gap-2"><Calculator className="w-8 h-8" /> 包租唐戰情室 <span className="text-xs bg-emerald-900/50 text-emerald-200 px-2 py-0.5 rounded border border-emerald-500/30">V45 Online</span></h1>
           <div className="flex items-center gap-4 mt-2">
               <div className="flex gap-2">
                   <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-xs shadow-sm">
                       ...
                       {saveStatus === 'idle' && (dataSrc === 'cloud' ? <><Wifi className="w-3 h-3 text-blue-400"/><span className="text-blue-400">雲端連線</span></> : <><WifiOff className="w-3 h-3 text-slate-500"/><span className="text-slate-500">本機模式</span></>)}
                   </div>
               </div>
           </div>
        </div>
        <div className="flex flex-wrap gap-2">
            ...
            <button onClick={handleUpdatePrices} disabled={isUpdatingPrices} className="flex items-center gap-2 px-3 py-2 bg-slate-800 hover:bg-emerald-900/50 border border-emerald-500/50 rounded-lg text-sm text-emerald-400 transition-all group">{isUpdatingPrices ? <Loader2 className="w-4 h-4 animate-spin"/> : <RefreshCw className="w-4 h-4 group-hover:rotate-180 transition-transform duration-700" />} 更新行情</button>
                                ...
                                <div className="flex justify-between items-center mb-2">
                                    <div className="flex items-center gap-2 flex-1">
                                        <div className="relative group/edit">
                                            <input type="text" value={etf.code || ''} onChange={(e) => updateEtf(idx, 'code', e.target.value)} className="absolute -top-5 left-0 text-[10px] bg-slate-800 text-slate-400 border border-slate-600 rounded px-1 w-16 focus:w-24 transition-all outline-none" placeholder="代號" />
