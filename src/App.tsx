...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const state = { etfs, loans, stockLoan, creditLoan, globalMarginLoan, taxStatus, allocation, collection, tokens, cloudConfig };
      localStorage.setItem('baozutang_v51', JSON.stringify(state));
  ...
  const unrealizedPL = etfs.reduce((a, e) => a + (Number(e.shares) * Number(e.currentPrice) - Number(e.shares) * Number(e.costPrice)), 0);
  ...
  const handleUpdatePrices = async () => {
    ...
    try {
        ...
        const newEtfs = etfs.map(e => { const code = e.code || e.id; return priceMap.has(code) ? { ...e, currentPrice: priceMap.get(code)! } : e; });
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
        try {
            ...
            if(data.etfs) { setEtfs(data.etfs); if(data.loans) setLoans(data.loans); setTaxStatus(data.taxStatus); setAllocation(data.allocation); alert('匯入成功！'); }
            ...
            <div className="space-y-4">
              {etfs.map((e, i) => (
                <div key={e.id} className="p-4 bg-slate-900 rounded-xl border border-slate-700 relative group">
                  ...
                  <div className="flex justify-between items-center mb-2"><input type="text" value={e.name} onChange={v => { const n = [...etfs]; n[i].name = v.target.value; setEtfs(n); }} className="bg-transparent font-bold text-white outline-none w-2/3"/><div className="flex gap-1"><button onClick={() => setActiveBuyId(activeBuyId === e.id ? null : e.id)} className="p-1 rounded bg-slate-800"><ShoppingCart size={14}/></button><button onClick={() => setExpandedEtfId(expandedEtfId === e.id ? null : e.id)} className="p-1 rounded bg-slate-800"><List size={14}/></button></div></div>
                  {activeBuyId === e.id && (<div className="mb-3 p-3 bg-emerald-900/20 rounded-lg"><div className="grid grid-cols-2 gap-2 mb-2"><input type="number" placeholder="股數" value={buyForm.shares} onChange={v => setBuyForm({...buyForm, shares: v.target.value})} className="bg-slate-900 p-1 rounded text-xs"/><input type="number" placeholder="單價" value={buyForm.price} onChange={v => setBuyForm({...buyForm, price: v.target.value})} className="bg-slate-900 p-1 rounded text-xs"/><input type="number" placeholder="融資額" value={buyForm.margin} onChange={v => setBuyForm({...buyForm, margin: v.target.value})} className="bg-slate-900 p-1 rounded text-xs"/><input type="date" value={buyForm.date} onChange={v => setBuyForm({...buyForm, date: v.target.value})} className="bg-slate-900 p-1 rounded text-xs"/></div><button onClick={() => {
                      const s = Number(buyForm.shares), p = Number(buyForm.price), m = Number(buyForm.margin); if (!s || !p) return;
                      const nEtfs = [...etfs]; const current = nEtfs[i];
                      const newLot = { id: Date.now().toString(), date: buyForm.date, shares: s, price: p, margin: m };
                      const updatedLots = current.lots ? [...current.lots, newLot] : [newLot];
                      nEtfs[i] = recalculateEtfStats({ ...current, lots: updatedLots });
                      setEtfs(nEtfs); setBuyForm({ shares: '', price: '', date: '', margin: '' }); setActiveBuyId(null);
                  }} className="w-full bg-emerald-600 text-xs py-1 rounded">確認</button></div>)}
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div><label className="text-slate-500">股數</label><div className="pt-1">{Number(e.shares).toLocaleString()}</div></div>
                    <div><label className="text-slate-500">現價</label><input type="number" value={e.currentPrice} onChange={v => { const n = [...etfs]; n[i].currentPrice = Number(v.target.value); setEtfs(n); }} className="w-full bg-slate-800 rounded p-1 border border-slate-700 mt-1"/></div>
              ...
              <button onClick={() => setEtfs([...etfs, { id: Date.now().toString(), name: '新標的', shares: 0, costPrice: 0, currentPrice: 0, dividendPerShare: 0, dividendType: 'annual', payMonths: [1,4,7,10], category: 'dividend', marginLoanAmount: 0 }])} className="w-full py-2 border border-dashed border-slate-600 rounded-xl text-slate-500 hover:text-white transition-all">+ 新增標的</button>
            </div>
          </section>
        </div>
             ...
             <table className="w-full text-sm text-left">
               ...
               <tbody>
                 {monthlyFlows.map(r => (
                   <tr key={r.month} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                     ...
                     <td className="p-3 text-slate-400 font-mono">{formatMoney(r.life)}</td>
                     <td className="p-3 text-purple-400 font-mono">{formatMoney(r.tax)}</td>
               ...
               <tfoot>
                 <tr className="bg-slate-900 font-black text-white">
                   ...
                   <td className="p-3 text-purple-400 font-mono">{formatMoney(totalTax)}</td>
                   <td className={`p-3 text-right font-bold ${totalNet >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{formatMoney(totalNet)}</td>
                 </tr>
               </tfoot>
             </table>
           </div>
