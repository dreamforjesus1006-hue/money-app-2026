  ...
  const handleScanTWSE = async () => {
    ...
    try {
      ...
      const fetchStrategies = [
        { name: '直連證交所', url: targetUrl, parse: async (res: Response) => await res.json() },
        { name: '備援線路 A', url: `https://api.allorigins.win/raw?url=${encodedTarget}`, parse: async (res: Response) => await res.json() },
        { name: '備援線路 B', url: `https://api.codetabs.com/v1/proxy?quest=${encodedTarget}`, parse: async (res: Response) => await res.json() }
      ];
      ...
      for (const strategy of fetchStrategies) {
        try {
          ...
          console.warn(`${strategy.name} 連線失敗`);
        }
      }

      if (!fetchSuccess || !data) {
        throw new Error('證交所防火牆阻擋');
      }
      ...
      setEtfs(prev => prev.map(etf => {
          ...
          if (existingIdx >= 0) {
              ...
              if (emptyIdx >= 0) {
                  ...
                  newSchedule.push({ id: `auto-${Date.now()}`, year, name: `${year} 證交所公告`, exDate: exDateStr, payDate: '', amount, isActual: false });
              }
          }
          updatedCount++;
      ...
      if (updatedCount > 0) {
          ...
          alert(`連線掃描成功！✅\n\n但目前證交所「除權息預告表」中，尚未有您手中 ETF 的近期配息公告。\n(註：投信通常在除息前 1~2 週才會送交資料，請過幾天再試！)`);
      }

    } catch(e) {
      alert('自動掃描失敗。\n\n【備用方案】: 請在您的 Google Sheet CSV 後方加入配息資料，然後點擊上方「綠色更新按鈕」即可自動匯入行事曆！\n格式: 代號, 現價, 配息金額, 除息日, 發放日');
    } finally {
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-slate-950 text-white font-sans selection:bg-emerald-500/30">
      <header className="mb-8 border-b border-slate-800 pb-4 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400 flex items-center gap-2 drop-shadow-md"><Calculator className="text-emerald-400"/> 包租唐戰情室 V93</h1>
      ...
      {/* V93 智能配息雷達 (Upcoming Events + Direct Scanner) */}
      <div className="mb-8">
