...
// V83 新增: dividendTaxableRatio (54C 比例)
...
type PersistedPayload = { etfs: ETF[]; loans: Loan[]; stockLoan: StockLoan; creditLoan: CreditLoan; globalMarginLoan: StockLoan; taxStatus: TaxStatus; allocation: AllocationConfig; cloudConfig: CloudConfig; actualDetails: ActualDetails; monthlyRecords?: MonthlyRecords; _meta?: { schema: number; updatedAt: number }; };
...
// V83: 預設應稅比例 30%
const DEFAULT_TAX: TaxStatus = { salaryIncome: 589200, livingExpenses: 0, hasSpouse: true, isDisabled: true, dependents: 0, disabilityCount: 1, dividendTaxableRatio: 30 };
...
const calculateIncomeTax = (salary: number, dividend: number, otherTaxable: number, status: TaxStatus) => {
  ...
  // 薪資扣除額：最多 21.8 萬 (簡化為單人薪資計算，因為您說這是金流用)
    ...
    etfs.forEach((e) => {
      ...
      if (yearEvents.length > 0) {
        yearEvents.forEach((event) => {
          ...
          if (e.lots && e.lots.length > 0) e.lots.forEach((lot) => { if (!Number.isFinite(exT) || toTime(lot.date) < exT) qualifiedShares += safeNum(lot.shares); });
...
const StorageService = {
  ...
  loadData: async () => {
    ...
    // V83 防呆：確保舊資料載入時有 dividendTaxableRatio
    if(picked.taxStatus && picked.taxStatus.dividendTaxableRatio === undefined) {
        picked.taxStatus.dividendTaxableRatio = 30;
    }

    const oldActuals = picked.actualDetails || picked.actuals || {};
    ...
    picked.actualDetails = newActuals;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setDataSrc(res.cloudOk ? 'cloud' : 'local'); setSaveStatus(res.cloudOk ? 'saved' : 'error');
  ...
  const monthlyFlows = useMemo(() => generateCashFlow(etfs, loans, stockLoan, creditLoan, globalMarginLoan, taxStatus, actualDetails, monthlyRecords, selectedYear), [etfs, loans, stockLoan, creditLoan, globalMarginLoan, taxStatus, actualDetails, monthlyRecords, selectedYear]);
    ...
    <div className="min-h-screen p-4 md:p-8 bg-slate-900 text-white font-sans selection:bg-emerald-500/30">
      <header className="mb-8 border-b border-slate-700 pb-4 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button onClick={async () => { if (!cloudConfig.priceSourceUrl) { alert('請輸入連結！'); setShowSettings(true); return; } setIsUpdatingPrices(true); try { const res = await fetch(cloudConfig.priceSourceUrl.includes('/edit') ? cloudConfig.priceSourceUrl.replace(/\/edit.*$/, '/export?format=csv') : cloudConfig.priceSourceUrl); const priceMap = parseCsvPriceMap(await res.text()); setEtfs((prev) => prev.map((e) => { const key = (e.code || e.id || '').trim(); return priceMap.has(key) ? { ...e, currentPrice: priceMap.get(key)! } : e; })); alert('成功！'); } catch (e) { alert('失敗'); } finally { setIsUpdatingPrices(false); } }} className="p-2 bg-slate-800 rounded text-emerald-400"><RefreshCw size={18} className={isUpdatingPrices ? "animate-spin" : ""} /></button>
          <button onClick={() => setShowSettings(true)} className="p-2 bg-slate-800 rounded"><Settings size={18} /></button>
          ...
          <button onClick={() => fileInputRef.current?.click()} className="p-2 bg-slate-800 rounded text-blue-400"><Upload size={18} /></button>
          <button onClick={() => StorageService.exportToFile({ etfs, loans, stockLoan, creditLoan, globalMarginLoan, taxStatus, allocation, cloudConfig, actualDetails, monthlyRecords })} className="p-2 bg-slate-800 rounded text-amber-400"><Download size={18} /></button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        <div className="xl:col-span-4 space-y-6">
          <div className="bg-slate-800 p-6 rounded-2xl border border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.15)] relative overflow-hidden">
            ...
            <div className="mb-6"><div className="flex justify-between text-[10px] text-slate-400 mb-1"><span>經驗 (往 {nextRank})</span><span>{progress.toFixed(1)}%</span></div><div className="w-full bg-slate-900 h-3 rounded-full overflow-hidden border border-slate-700 shadow-inner"><div className="bg-gradient-to-r from-emerald-600 to-emerald-400 h-full relative" style={{ width: `${progress}%` }}></div></div></div>
            <div className="grid grid-cols-4 gap-2 border-t border-slate-700 pt-4">
              ...
              <div className="text-center bg-slate-900/50 p-2 rounded-lg"><div className="text-slate-500 text-[9px] mb-1">防禦力(維持率)</div><div className={`font-bold text-sm ${currentMaintenance < 140 ? 'text-red-400' : 'text-blue-400'}`}>{currentMaintenance === 999 ? 'MAX' : currentMaintenance.toFixed(0) + '%'}</div></div>
                          ...
                          yearEvents.map((event) => (
                            <div key={event.id} className="grid grid-cols-7 gap-2 text-[10px] items-center bg-slate-900/50 p-2 rounded">
                              ...
                              <div className="col-span-5 grid grid-cols-3 gap-2">
                                <div><div className="text-slate-500 mb-0.5 text-[8px]">除息日</div><input type="date" value={event.exDate} onChange={(v) => setEtfs(prev => prev.map(etf => etf.id === e.id ? {...etf, schedule: (etf.schedule||[]).map(ev => ev.id === event.id ? {...ev, exDate: v.target.value} : ev)} : etf))} className="w-full bg-slate-900 rounded p-1 text-slate-300" /></div>
                                <div><div className="text-slate-500 mb-0.5 text-[8px]">發放日</div><input type="date" value={event.payDate} onChange={(v) => setEtfs(prev => prev.map(etf => etf.id === e.id ? {...etf, schedule: (etf.schedule||[]).map(ev => ev.id === event.id ? {...ev, payDate: v.target.value} : ev)} : etf))} className="w-full bg-slate-900 rounded p-1 text-emerald-400" /></div>
                                <div><div className="text-slate-500 mb-0.5 text-[8px]">金額</div><input type="number" step="0.01" value={event.amount} onChange={(v) => setEtfs(prev => prev.map(etf => etf.id === e.id ? {...etf, schedule: (etf.schedule||[]).map(ev => ev.id === event.id ? {...ev, amount: safeNum(v.target.value)} : ev)} : etf))} className="w-full bg-slate-900 rounded p-1 text-right text-yellow-400" /></div>
                              </div>
                            </div>
                          ))
                        ) : (
                          <button onClick={() => setEtfs(prev => prev.map(etf => etf.id === e.id ? {...etf, schedule: [...(etf.schedule||[]), ...[1,2,3,4].map(q=>({id:`${Date.now()}-q${q}`, year: selectedYear, name:`${selectedYear} Q${q}`, exDate:'', payDate:'', amount:safeNum(etf.dividendPerShare), isActual:false}))]} : etf))} className="px-5 py-2.5 bg-emerald-600 text-white text-sm font-bold rounded-lg w-full">➕ 建立行事曆</button>
                        )}
                      </div>
                    </div>
                  )}
                ...
                {monthlyFlows.map((r) => (
                  <React.Fragment key={r.month}>
                    <tr className="border-b border-slate-700/50 hover:bg-slate-700/30 font-mono text-xs cursor-pointer" onClick={() => setExpandedMonth(expandedMonth === r.month ? null : r.month)}>
                      ...
                      <td className="p-3 text-purple-400 text-[10px]">{formatMoney(r.healthTax)}</td>
                      ...
                      <tr className="bg-slate-900/80">
                        <td colSpan={11} className="p-4 border-b border-emerald-900/30">
                          ...
                          <div className="space-y-3 max-w-4xl">
                            <div className="text-sm text-emerald-400 font-bold border-b border-slate-700 pb-2"> 實領輸入</div>
                  ...
                  <div className="space-y-2 p-3 bg-slate-900 rounded-xl">
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                            <label className="text-slate-400 text-xs">ETF 股息應稅比例 (54C)</label>
                            <span className="text-yellow-400 font-bold text-xs">{taxStatus.dividendTaxableRatio ?? 30}%</span>
                        </div>
                        ...
                        <div className="text-[10px] text-slate-500 mt-1">高股息ETF多數包含免稅平準金，預設30%應稅較貼近真實。</div>
                    </div>
                    <div><label className="text-slate-400 text-xs">年薪資 (僅計稅，不入淨流)</label><input type="number" value={taxStatus.salaryIncome} onChange={(e) => setTaxStatus({ ...taxStatus, salaryIncome: safeNum(e.target.value) })} className="w-full bg-slate-800 p-1.5 rounded" /></div>
